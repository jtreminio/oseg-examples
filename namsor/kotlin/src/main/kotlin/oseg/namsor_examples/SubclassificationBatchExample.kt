...
class SubclassificationBatchExample
{
    fun subclassificationBatch()
    {
        ...
        try
        {
            val response = PersonalApi().subclassificationBatch(
                batchFirstLastNameGeoIn = batchFirstLastNameGeoIn,
            )

            println(response)
        } catch (e: ClientException) {
            println("4xx response calling PersonalApi#subclassificationBatch")
            e.printStackTrace()
        } catch (e: ServerException) {
            println("5xx response calling PersonalApi#subclassificationBatch")
            e.printStackTrace()
        }
    }
}
