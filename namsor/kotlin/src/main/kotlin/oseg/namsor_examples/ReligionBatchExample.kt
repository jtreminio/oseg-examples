...
class ReligionBatchExample
{
    fun religionBatch()
    {
        ...
        try
        {
            val response = PersonalApi().religionBatch(
                batchFirstLastNameGeoSubdivisionIn = batchFirstLastNameGeoSubdivisionIn,
            )

            println(response)
        } catch (e: ClientException) {
            println("4xx response calling PersonalApi#religionBatch")
            e.printStackTrace()
        } catch (e: ServerException) {
            println("5xx response calling PersonalApi#religionBatch")
            e.printStackTrace()
        }
    }
}
