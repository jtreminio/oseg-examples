...
public class BatchCommunityEngageFullOut {
  ...
  public BatchCommunityEngageFullOut engagementCandidates(@javax.annotation.Nullable List<CommunityEngageOut> engagementCandidates) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to BatchCommunityEngageFullOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BatchCommunityEngageFullOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BatchCommunityEngageFullOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BatchCommunityEngageFullOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
