...
public class BatchFirstLastNameOriginedOut {
  ...
  private List<FirstLastNameOriginedOut> personalNames = new ArrayList<>();

  public BatchFirstLastNameOriginedOut() {
  }

  public BatchFirstLastNameOriginedOut personalNames(@javax.annotation.Nullable List<FirstLastNameOriginedOut> personalNames) {
  ...
  public void setPersonalNames(@javax.annotation.Nullable List<FirstLastNameOriginedOut> personalNames) {
    this.personalNames = personalNames;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to BatchFirstLastNameOriginedOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BatchFirstLastNameOriginedOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BatchFirstLastNameOriginedOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BatchFirstLastNameOriginedOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("personalNames") != null && !jsonObj.get("personalNames").isJsonNull()) {
        ...
        if (jsonArraypersonalNames != null) {
          ...
          for (int i = 0; i < jsonArraypersonalNames.size(); i++) {
            FirstLastNameOriginedOut.validateJsonElement(jsonArraypersonalNames.get(i));
          };
        }
      }
  }
