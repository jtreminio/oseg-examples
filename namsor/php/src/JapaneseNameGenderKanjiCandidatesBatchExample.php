...
$personal_names_1 = (new Namsor\Client\Model\FirstLastNameGenderIn())
    ...
    ->setFirstName("Takashi")
    ->setLastName("Murakami")
    ->setGender("male");
...
try {
    $response = (new Namsor\Client\Api\JapaneseApi(config: $config))->japaneseNameGenderKanjiCandidatesBatch(
        batch_first_last_name_gender_in: $batch_first_last_name_gender_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling JapaneseApi#japaneseNameGenderKanjiCandidatesBatch: {$e->getMessage()}";
}
