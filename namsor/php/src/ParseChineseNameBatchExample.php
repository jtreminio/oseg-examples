...
try {
    $response = (new Namsor\Client\Api\ChineseApi(config: $config))->parseChineseNameBatch(
        batch_personal_name_in: $batch_personal_name_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling ChineseApi#parseChineseNameBatch: {$e->getMessage()}";
}
