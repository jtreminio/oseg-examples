...
try {
    $response = (new Namsor\Client\Api\ChineseApi(config: $config))->chineseNameMatchBatch(
        batch_match_personal_first_last_name_in: $batch_match_personal_first_last_name_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling ChineseApi#chineseNameMatchBatch: {$e->getMessage()}";
}
