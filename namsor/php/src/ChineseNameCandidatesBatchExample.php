...
try {
    $response = (new Namsor\Client\Api\ChineseApi(config: $config))->chineseNameCandidatesBatch(
        batch_first_last_name_in: $batch_first_last_name_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling ChineseApi#chineseNameCandidatesBatch: {$e->getMessage()}";
}
