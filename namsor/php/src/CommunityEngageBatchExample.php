...
try {
    $response = (new Namsor\Client\Api\PersonalApi(config: $config))->communityEngageBatch(
        batch_first_last_name_geo_in: $batch_first_last_name_geo_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling PersonalApi#communityEngageBatch: {$e->getMessage()}";
}
