...
try {
    $response = (new Namsor\Client\Api\JapaneseApi(config: $config))->japaneseNameLatinCandidatesBatch(
        batch_first_last_name_in: $batch_first_last_name_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling JapaneseApi#japaneseNameLatinCandidatesBatch: {$e->getMessage()}";
}
