...
try {
    $response = (new Namsor\Client\Api\PersonalApi(config: $config))->subclassificationFullBatch(
        batch_personal_name_geo_in: $batch_personal_name_geo_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling PersonalApi#subclassificationFullBatch: {$e->getMessage()}";
}
