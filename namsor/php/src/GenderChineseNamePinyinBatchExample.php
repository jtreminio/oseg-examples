...
try {
    $response = (new Namsor\Client\Api\ChineseApi(config: $config))->genderChineseNamePinyinBatch(
        batch_first_last_name_in: $batch_first_last_name_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling ChineseApi#genderChineseNamePinyinBatch: {$e->getMessage()}";
}
