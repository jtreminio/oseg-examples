...
try {
    $response = (new Namsor\Client\Api\ChineseApi(config: $config))->pinyinChineseNameBatch(
        batch_personal_name_in: $batch_personal_name_in,
    );

    print_r($response);
} catch (Namsor\Client\ApiException $e) {
    echo "Exception when calling ChineseApi#pinyinChineseNameBatch: {$e->getMessage()}";
}
