...
$integration_delivery_configuration_post = (new LaunchDarkly\Client\Model\IntegrationDeliveryConfigurationPost())
    ...
    EOD, true))
    ->setOn(false)
    ->setName("Example persistent store integration")
    ->setTags([
        "example-tag",
    ]);

try {
    $response = (new LaunchDarkly\Client\Api\PersistentStoreIntegrationsBetaApi(config: $config))->createBigSegmentStoreIntegration(
