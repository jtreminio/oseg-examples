...
import com.launchdarkly.client.model.UrlPost;
...
public class MetricPost {
  ...
  public MetricPost isActive(@javax.annotation.Nullable Boolean isActive) {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(key, name, description, kind, selector, urls, isActive, isNumeric, unit, eventKey, successCriteria, tags, randomizationUnits, unitAggregationType, analysisType, percentileValue, eventDefault);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MetricPost.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MetricPost` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
