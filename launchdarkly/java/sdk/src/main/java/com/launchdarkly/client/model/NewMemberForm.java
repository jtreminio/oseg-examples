...
public class NewMemberForm {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(email, password, firstName, lastName, role, customRoles, teamKeys, roleAttributes);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!NewMemberForm.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `NewMemberForm` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
