...
public class DependentMetricGroupRepWithMetrics {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(key, name, kind, links, metrics);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DependentMetricGroupRepWithMetrics.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DependentMetricGroupRepWithMetrics` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
