...
public class ExpandedFlagRep {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(name, kind, description, key, version, creationDate, includeInSnippet, clientSideAvailability, variations, temporary, tags, links, maintainerId, maintainer, customProperties, archived, archivedDate, defaults);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ExpandedFlagRep.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ExpandedFlagRep` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
