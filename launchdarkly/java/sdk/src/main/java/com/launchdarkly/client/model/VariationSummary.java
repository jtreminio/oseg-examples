...
public class VariationSummary {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(rules, nullRules, targets, contextTargets, isFallthrough, isOff, rollout, bucketBy);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VariationSummary.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `VariationSummary` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
