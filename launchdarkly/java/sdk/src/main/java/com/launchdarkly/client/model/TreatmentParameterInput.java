...
public class TreatmentParameterInput {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(flagKey, variationId);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TreatmentParameterInput.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TreatmentParameterInput` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
