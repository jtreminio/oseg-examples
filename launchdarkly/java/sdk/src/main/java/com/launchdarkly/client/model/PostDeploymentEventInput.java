...
public class PostDeploymentEventInput {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(projectKey, environmentKey, applicationKey, applicationName, applicationKind, version, versionName, eventType, eventTime, eventMetadata, deploymentMetadata);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PostDeploymentEventInput.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PostDeploymentEventInput` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
