...
import com.launchdarkly.client.model.PullRequestLeadTimeRep;
...
public class PullRequestRep {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(id, externalId, title, status, author, createTime, mergeTime, mergeCommitKey, baseCommitKey, headCommitKey, filesChanged, linesAdded, linesDeleted, url, deployments, flagReferences, leadTime);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PullRequestRep.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PullRequestRep` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
