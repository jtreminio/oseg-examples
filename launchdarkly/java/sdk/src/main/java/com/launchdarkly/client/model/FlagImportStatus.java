...
import com.launchdarkly.client.model.StatusResponse;
...
public class FlagImportStatus {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(status, lastImport, lastError, errors);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FlagImportStatus.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FlagImportStatus` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
