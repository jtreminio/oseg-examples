...
public class PatchSegmentExpiringTargetInstruction {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(kind, contextKey, contextKind, targetType, value, version);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PatchSegmentExpiringTargetInstruction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PatchSegmentExpiringTargetInstruction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
