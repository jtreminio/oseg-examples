...
public class StatisticRep {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(name, type, sourceLink, defaultBranch, enabled, version, hunkCount, fileCount, links, latestCommitTime);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!StatisticRep.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `StatisticRep` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
