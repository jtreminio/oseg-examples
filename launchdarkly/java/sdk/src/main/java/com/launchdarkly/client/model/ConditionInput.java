...
public class ConditionInput {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(scheduleKind, executionDate, waitDuration, waitDurationUnit, executeNow, description, notifyMemberIds, notifyTeamKeys, kind);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ConditionInput.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ConditionInput` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
