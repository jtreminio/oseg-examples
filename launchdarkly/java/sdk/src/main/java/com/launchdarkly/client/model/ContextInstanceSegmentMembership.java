...
public class ContextInstanceSegmentMembership {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(name, key, description, unbounded, external, isMember, isIndividuallyTargeted, isRuleTargeted, links);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ContextInstanceSegmentMembership.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ContextInstanceSegmentMembership` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
