...
public class HoldoutPostRequest {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(name, key, description, randomizationunit, attributes, holdoutamount, primarymetrickey, metrics, prerequisiteflagkey, maintainerId);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!HoldoutPostRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HoldoutPostRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
