...
public class MetricInGroupRep {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(key, versionId, name, kind, isNumeric, unitAggregationType, links, nameInGroup, randomizationUnits);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MetricInGroupRep.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MetricInGroupRep` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
