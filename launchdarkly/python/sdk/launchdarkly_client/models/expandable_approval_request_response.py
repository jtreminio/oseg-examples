...
class ExpandableApprovalRequestResponse(BaseModel):
    """
    ExpandableApprovalRequestResponse
    """ # noqa: E501
    id: StrictStr = Field(description="The ID of this approval request", alias="_id")
    version: StrictInt = Field(description="Version of the approval request", alias="_version")
    creation_date: StrictInt = Field(alias="creationDate")
    service_kind: StrictStr = Field(alias="serviceKind")
    requestor_id: Optional[StrictStr] = Field(default=None, description="The ID of the member who requested the approval", alias="requestorId")
    description: Optional[StrictStr] = Field(default=None, description="A human-friendly name for the approval request")
    review_status: StrictStr = Field(description="Current status of the review of this approval request", alias="reviewStatus")
    all_reviews: List[ReviewResponse] = Field(description="An array of individual reviews of this approval request", alias="allReviews")
    notify_member_ids: List[StrictStr] = Field(description="An array of member IDs. These members are notified to review the approval request.", alias="notifyMemberIds")
    applied_date: Optional[StrictInt] = Field(default=None, alias="appliedDate")
    applied_by_member_id: Optional[StrictStr] = Field(default=None, description="The member ID of the member who applied the approval request", alias="appliedByMemberId")
    applied_by_service_token_id: Optional[StrictStr] = Field(default=None, description="The service token ID of the service token which applied the approval request", alias="appliedByServiceTokenId")
    status: StrictStr = Field(description="Current status of the approval request")
    instructions: List[Dict[str, Any]]
    conflicts: List[Conflict] = Field(description="Details on any conflicting approval requests")
    links: Dict[str, Any] = Field(description="The location and content type of related resources", alias="_links")
    execution_date: Optional[StrictInt] = Field(default=None, alias="executionDate")
    operating_on_id: Optional[StrictStr] = Field(default=None, description="ID of scheduled change to edit or delete", alias="operatingOnId")
    integration_metadata: Optional[IntegrationMetadata] = Field(default=None, alias="integrationMetadata")
    source: Optional[CopiedFromEnv] = None
    custom_workflow_metadata: Optional[CustomWorkflowMeta] = Field(default=None, alias="customWorkflowMetadata")
    resource_id: Optional[StrictStr] = Field(default=None, description="String representation of a resource", alias="resourceId")
    approval_settings: Optional[ApprovalSettings] = Field(default=None, alias="approvalSettings")
    project: Optional[Project] = None
    environments: Optional[List[Environment]] = Field(default=None, description="List of environments the approval impacts")
    flag: Optional[ExpandedFlagRep] = None
    resource: Optional[ExpandedResourceRep] = None
    __properties: ClassVar[List[str]] = ["_id", "_version", "creationDate", "serviceKind", "requestorId", "description", "reviewStatus", "allReviews", "notifyMemberIds", "appliedDate", "appliedByMemberId", "appliedByServiceTokenId", "status", "instructions", "conflicts", "_links", "executionDate", "operatingOnId", "integrationMetadata", "source", "customWorkflowMetadata", "resourceId", "approvalSettings", "project", "environments", "flag", "resource"]

    @field_validator('review_status')
    def review_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['approved', 'declined', 'pending']):
            raise ValueError("must be one of enum values ('approved', 'declined', 'pending')")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pending', 'completed', 'failed', 'scheduled']):
            raise ValueError("must be one of enum values ('pending', 'completed', 'failed', 'scheduled')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.integration_metadata:
            _dict['integrationMetadata'] = self.integration_metadata.to_dict()
        # override the default output from pydantic by calling `to_dict()` of source
        if self.source:
            _dict['source'] = self.source.to_dict()
        # override the default output from pydantic by calling `to_dict()` of custom_workflow_metadata
        if self.custom_workflow_metadata:
            _dict['customWorkflowMetadata'] = self.custom_workflow_metadata.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "allReviews": [ReviewResponse.from_dict(_item) for _item in obj["allReviews"]] if obj.get("allReviews") is not None else None,
            "notifyMemberIds": obj.get("notifyMemberIds"),
            "appliedDate": obj.get("appliedDate"),
            "appliedByMemberId": obj.get("appliedByMemberId"),
            "appliedByServiceTokenId": obj.get("appliedByServiceTokenId"),
            "status": obj.get("status"),
            "instructions": obj.get("instructions"),
            "conflicts": [Conflict.from_dict(_item) for _item in obj["conflicts"]] if obj.get("conflicts") is not None else None,
            "_links": obj.get("_links"),
            "executionDate": obj.get("executionDate"),
            "operatingOnId": obj.get("operatingOnId"),
            "integrationMetadata": IntegrationMetadata.from_dict(obj["integrationMetadata"]) if obj.get("integrationMetadata") is not None else None,
            "source": CopiedFromEnv.from_dict(obj["source"]) if obj.get("source") is not None else None,
            "customWorkflowMetadata": CustomWorkflowMeta.from_dict(obj["customWorkflowMetadata"]) if obj.get("customWorkflowMetadata") is not None else None,
            "resourceId": obj.get("resourceId"),
            "approvalSettings": ApprovalSettings.from_dict(obj["approvalSettings"]) if obj.get("approvalSettings") is not None else None,
            ...
            "flag": ExpandedFlagRep.from_dict(obj["flag"]) if obj.get("flag") is not None else None,
            "resource": ExpandedResourceRep.from_dict(obj["resource"]) if obj.get("resource") is not None else None
        })
        return _obj
