/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package com.launchdarkly.client.models

import com.launchdarkly.client.models.Link
import com.launchdarkly.client.models.SimpleHoldoutRep

import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * 
 *
 * @param items 
 * @param links The location and content type of related resources
 * @param totalCount The total number of holdouts in this project and environment.
 */


data class HoldoutsCollectionRep (

    @Json(name = "items")
    val items: kotlin.collections.List<SimpleHoldoutRep>? = null,

    /* The location and content type of related resources */
    @Json(name = "_links")
    val links: kotlin.collections.Map<kotlin.String, Link>? = null,

    /* The total number of holdouts in this project and environment. */
    @Json(name = "total_count")
    val totalCount: kotlin.Int? = null

) {


}

