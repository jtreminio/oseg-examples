/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package com.launchdarkly.client.models

import com.launchdarkly.client.models.InsightsRepositoryProject
import com.launchdarkly.client.models.Link

import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * 
 *
 * @param totalCount Total number of repository project associations
 * @param items List of repository project associations
 * @param links The location and content type of related resources
 */


data class InsightsRepositoryProjectCollection (

    /* Total number of repository project associations */
    @Json(name = "totalCount")
    val totalCount: kotlin.Int,

    /* List of repository project associations */
    @Json(name = "items")
    val items: kotlin.collections.List<InsightsRepositoryProject>,

    /* The location and content type of related resources */
    @Json(name = "_links")
    val links: kotlin.collections.Map<kotlin.String, Link>? = null

) {


}

