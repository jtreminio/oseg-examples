/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package com.launchdarkly.client.models

import com.launchdarkly.client.models.Link
import com.launchdarkly.client.models.MemberSummary

import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * 
 *
 * @param totalCount The number of maintainers of the team
 * @param items Details on the members that have been assigned as maintainers of the team
 * @param links The location and content type of related resources
 */


data class TeamMaintainers (

    /* The number of maintainers of the team */
    @Json(name = "totalCount")
    val totalCount: kotlin.Int? = null,

    /* Details on the members that have been assigned as maintainers of the team */
    @Json(name = "items")
    val items: kotlin.collections.List<MemberSummary>? = null,

    /* The location and content type of related resources */
    @Json(name = "_links")
    val links: kotlin.collections.Map<kotlin.String, Link>? = null

) {


}

