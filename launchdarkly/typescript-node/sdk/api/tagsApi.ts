...
export class TagsApi {
     ...
     * Get a list of tags.
     * @summary List tags
     * @param kind Fetch tags associated with the specified resource type. Options are &#x60;flag&#x60;, &#x60;project&#x60;, &#x60;environment&#x60;, &#x60;segment&#x60;, &#x60;metric&#x60;. Returns all types by default.
     * @param pre Return tags with the specified prefix
     * @param archived Whether or not to return archived flags
     * @param limit The number of tags to return. Maximum is 1000.
     * @param offset The index of the first tag to return. Default is 0.
     * @param asOf The time to retrieve tags as of. Default is the current time.
     */
    public async getTags (kind?: Array<string>, pre?: string, archived?: boolean, limit?: number, offset?: number, asOf?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TagsCollection;  }> {
        ...
        if (kind !== undefined) {
            localVarQueryParameters['kind'] = ObjectSerializer.serialize(kind, "Array<string>");
        }

        if (pre !== undefined) {
            localVarQueryParameters['pre'] = ObjectSerializer.serialize(pre, "string");
        }
        ...
        if (asOf !== undefined) {
            localVarQueryParameters['asOf'] = ObjectSerializer.serialize(asOf, "string");
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: TagsCollection;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "TagsCollection");
