...
export class UsersApi {
     ...
     * @deprecated
     */
    public async deleteUser (projectKey: string, environmentKey: string, userKey: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body?: any;  }> {
        const localVarPath = this.basePath + '/api/v2/users/{projectKey}/{environmentKey}/{userKey}'
     ...
     * @param searchAfter Limits results to users with sort values after the value you specify. You can use this for pagination, but we recommend using the &#x60;next&#x60; link we provide instead.
     * @param filter A comma-separated list of user attribute filters. Each filter is in the form of attributeKey:attributeValue
     *
     * @deprecated
     */
    public async getSearchUsers (projectKey: string, environmentKey: string, q?: string, limit?: number, offset?: number, after?: number, sort?: string, searchAfter?: string, filter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Users;  }> {
        const localVarPath = this.basePath + '/api/v2/user-search/{projectKey}/{environmentKey}'
            .replace('{' + 'projectKey' + '}', encodeURIComponent(String(projectKey)))
            .replace('{' + 'environmentKey' + '}', encodeURIComponent(String(environmentKey)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        if (searchAfter !== undefined) {
            localVarQueryParameters['searchAfter'] = ObjectSerializer.serialize(searchAfter, "string");
        }
     ...
     * @deprecated
     */
    public async getUser (projectKey: string, environmentKey: string, userKey: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: UserRecord;  }> {
        const localVarPath = this.basePath + '/api/v2/users/{projectKey}/{environmentKey}/{userKey}'
            .replace('{' + 'projectKey' + '}', encodeURIComponent(String(projectKey)))
            .replace('{' + 'environmentKey' + '}', encodeURIComponent(String(environmentKey)))
            .replace('{' + 'userKey' + '}', encodeURIComponent(String(userKey)));
     ...
     * @deprecated
     */
    public async getUsers (projectKey: string, environmentKey: string, limit?: number, searchAfter?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: UsersRep;  }> {
        const localVarPath = this.basePath + '/api/v2/users/{projectKey}/{environmentKey}'
            .replace('{' + 'projectKey' + '}', encodeURIComponent(String(projectKey)))
            .replace('{' + 'environmentKey' + '}', encodeURIComponent(String(environmentKey)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: UsersRep;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "UsersRep");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
